...
categoryRouter.put(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    if (foundCategory) {
      ...
      foundCategory.parentId = parentId || foundCategory.parentId;
      if (req.file?.filename && req.file.originalname != "null") {
        try {
          if (fs.existsSync(fileImageRoot(foundCategory.categoryImage))) {
            fs.unlink(
          ...
          foundCategory.categoryImage =
            req.file.filename || foundCategory.categoryImage;
...
// async function test() {
//   const allCategories = await Category.find({});
//   allCategories.forEach((item, index) => (item.order = index + 1));
